...
!************************************************************************************************************************************
!************************************************************************************************************************************
nf_errors = 100.; inewton = 0; ibhhh = 0; mome1 = 0.; mome2 = 0.; adam1 = 0.9; adam2 = 0.999
if (myid==0) then
    ...
    paraloop: do ibeta = 1, 64
        ...
        if (ibeta>1) then
            ...
            !else
                !delta(ibeta-1) = 0.01
            !endif
            delta(ibeta-1) = max(abs(theta_grad(ibeta-1))/100, 0.01)
        ...
        betap = theta_grad(30:34); muf = theta_grad(35:42); mup = theta_grad(43:50); betat = abs(theta_grad(51:53))
        sig_labf = abs(theta_grad(54)); sig_labp = abs(theta_grad(55))
        am = theta_grad(56); ak = theta_grad(57); aw = theta_grad(58); ae = theta_grad(59); etah = abs(theta_grad(60))
        lamdam = theta_grad(61:63)
        
        betam(2) = abs(betam(2)); betak(4) = abs(betak(4)); betak(7) = abs(betak(7)); betak(8) = -abs(betak(8))
        betaf(4:5) = abs(betaf(4:5)); betap(4:5) = abs(betap(4:5))
        muf(2) = abs(muf(2)); muf(4:6) = abs(muf(4:6)); muf(7:8) = -abs(muf(7:8))
        mup(2) = abs(mup(2)); mup(4:6) = abs(mup(4:6)); mup(7:8) = -abs(mup(7:8))
        
        penalty2 = 0
        if (etah>1) then; penalty2 = (etah-1)*10; etah = 1.; endif
...
!************************************************************************************************************************************
                    ...
                    logl_it(t) = logit_prob + logprobt + lprobt + logmprob
                endif ! end if1
...
!************************************************************************************************************************************
            ...
            logli_sect(ibeta, iagent) = logli_sect(ibeta, iagent)-penalty2
...
!************************************************************************************************************************************
    ...
    if (myid==0) then
        ...
        if (ifder>0) then
            ...
            ! step length
            step_size = 0.01
            !if (inewton>50) then; step_size = 0.05; endif
...
subroutine u_fp(e, ufp)
implicit none
integer e
real, intent(out):: ufp(2)

ufp(1) = dot_product(betaf, (/1., real(mful), real(e), real(blk), real(hsp)/))
ufp(2) = dot_product(betap, (/1., real(mpar), real(e), real(blk), real(hsp)/))
end subroutine u_fp
!**********************************************************
subroutine u_b(b, ub)
implicit none
...
if (knum>0 .and. kage<18) then
    if (kage<5) then
        kcost = knum*0.0101*inc
    else
        kcost = knum*0.0067*inc
    endif
endif
if (f==0 .and. p==0) then
    kcost = 0.
...
!********************************************************************************************************************************** 
function integ0f(m, knum, wh, wfp, ufp, emaxufp, nufp)
integer:: m
real:: knum, wh, wfp, ufp, emaxufp, nufp, integ0f
real:: normcdf, aux1, aux2, logitpdf

if (m==1) then
    aux1 = log(exp(-betam(3)-betak(5)*knum-ufp+emaxufp+nufp)-1)+log(wh)-log(wfp)
else
    aux1 = -betak(5)*knum-ufp+emaxufp-log(wfp)+nufp
endif
call vscdfnorm(1, aux1/sig_labf, normcdf)

call vscosh(1, nufp/2, aux2)
logitpdf = (1/(aux2*aux2))/4

integ0f = normcdf*logitpdf
end function integ0f
!**********************************************************************************************************************************
function integ0p(m, knum, wh, wfp, ufp, emaxufp, nufp)
integer:: m
real:: knum, wh, wfp, ufp, emaxufp, nufp, integ0p
real:: normcdf, aux1, aux2, logitpdf

if (m==1) then
    aux1 = log(exp(-betam(4)-betak(6)*knum-ufp+emaxufp+nufp)-1)+log(wh)-log(wfp)
else
    aux1 = -betak(6)*knum-ufp+emaxufp-log(wfp)+nufp
endif
call vscdfnorm(1, aux1/sig_labp, normcdf)

call vscosh(1, nufp/2, aux2)
logitpdf = (1/(aux2*aux2))/4

integ0p = normcdf*logitpdf
end function integ0p
!**********************************************************************************************************************************
function integf1(m, knum, wh, wf, wp, uf, up, emaxufp, nufp)
integer:: m
real:: knum, wh, wf, wp, uf, up, emaxufp, nufp, integf1
real:: normcdf, aux1, aux2, logitpdf

if (m==1) then
    aux1 = log((wh+wf)*exp(betam(3)-betam(4)+(betak(5)-betak(6))*knum+uf-up+emaxufp+nufp)-wh)-log(wp)
else
    aux1 = (betak(5)-betak(6))*knum+uf-up+emaxufp+log(wf)-log(wp)+nufp
endif
call vscdfnorm(1, aux1/sig_labp, normcdf)
...
integf1 = normcdf*logitpdf
end function integf1
!**********************************************************************************************************************************
function integp1(m, knum, wh, wf, wp, uf, up, emaxufp, nufp)
integer:: m
real:: knum, wh, wf, wp, uf, up, emaxufp, nufp, integp1
real:: normcdf, aux1, aux2, logitpdf

if (m==1) then
    aux1 = log((wh+wp)*exp(betam(4)-betam(3)+(betak(6)-betak(5))*knum+up-uf+emaxufp+nufp)-wh)-log(wf)
else
    aux1 =  (betak(6)-betak(5))*knum+up-uf+emaxufp+log(wp)-log(wf)+nufp
endif
call vscdfnorm(1, aux1/sig_labf, normcdf)
...
integp1 = normcdf*logitpdf
end function integp1
!**********************************************************************************************************************************
